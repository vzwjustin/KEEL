...
Reads Claude Code's stdin JSON payload for session info (model, context window,
workspace) and combines it with KEEL status. Works standalone — no GSD or other
plugins required.
"""
...
def _render_session(payload: dict, repo_root: Path) -> str:
    """Render model, folder, and context bar from Claude Code's stdin payload."""
